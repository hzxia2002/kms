...
<div id="surQuestionLayout" style="width:100%;overflow-y: hidden;overflow-x: hidden;margin:0; padding:0;">
    <div position="left"  title="调查题目树" id="accordion1">
        <ul id="surQuestionTree" style="margin-top:3px;"></ul>
    </div>
    <div position="center">
        ...
        <div id="surQuestionGrid" checkbox="true" rownumbers="true"  height="100%"  url='${ctx}/surQuestion/grid.do' root="rows" record="records"  width="100%"  colDraggable="true" >
        </div>
...
<script type="text/javascript">
    $(function(){
        //布局
        $("#surQuestionLayout").juiceLayout({ leftWidth: 190 , onAfterResize:function(width){
            $.jui.get("surQuestionGrid").setWidth(width);
            $.jui.get("surQuestionQuery").setWidth(width);
        }
        });
        //查询框初始化
        $("#surQuestionQuery").juicePanel({ collapse:false,height: 50,closeAble:false});
        //grid工具栏
        var toolbar={
            align:"right",
            items: [
...
//                        { text: '查看', icon:'view', click:function(item){doView(treeNode.data.uid)}},
     ...
     function doView(id) {
        ...
        commonView(url, "surQuestionGrid");
    }

    function doDelete(){
        commonDelete("surQuestionGrid","${ctx}/surQuestion/delete.do",refreshRootNode);
    }

    function doAdd(){        
        var url = "${ctx}/surQuestion/init.do";
        commonAddOrUpdate(url,"surQuestionGrid",null,"surQuestionGridForm",{title:"新增调查题目",height:530,width:600});
    }
    ...
    function doEdit(id){
        ...
        if(typeof id == "string"){
            ...
            url = initUrl(url,"surQuestionGrid");
        }
        if(url){
            commonAddOrUpdate(url,"surQuestionGrid",null,"surQuestionForm",{title:"编辑调查题目",height:530,width:600});
        }
    }
